...
import dgtic.unam.bluethooth.databinding.ActivityMainBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        MainActivity.iUpdate = object : Handler(Looper.getMainLooper()) {
            override fun handleMessage(msg: Message) {
                ...
                var cadena = String(buffer).substring(
                    0,
                    bytesReceived
                )
...
//registar los receptores de difusion,si no detecta cambiar ACTION_FOUND var filterUno = IntentFilter(BluetoothDevice.ACTION_NAME_CHANGED)
    ...
    inner class MyBroadCastReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (BluetoothDevice.ACTION_NAME_CHANGED.equals(action)) {
                ...
                if (!discoveryDevices.contains(
                        device
                    )
                ) {
                    discoveryDevices.add(device!!)
                    if (ActivityCompat.checkSelfPermission(
                            binding.root.context, Manifest.permission.BLUETOOTH_CONNECT
                        ) != PackageManager.PERMISSION_GRANTED
                    ) {
